...
def simulate_period_three_level(
    ...
    def evolve_3(rho3_in: np.ndarray) -> np.ndarray:
        rho = np.asarray(rho3_in, dtype=complex)
        squeezed = False
        if rho.ndim == 2:
            rho = rho[None, :, :]
            squeezed = True
    ...
    basis2 = np.eye(4, dtype=complex).reshape(4, 2, 2)
    rho3_basis = np.zeros((len(basis2), 3, 3), dtype=complex)
    rho3_basis[:, 0:2, 0:2] = basis2
    evolved_basis = evolve_3(rho3_basis)[:, 0:2, 0:2]
    super_matrix = evolved_basis.reshape(len(basis2), -1).T
